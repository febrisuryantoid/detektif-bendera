  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-pop-in">
      <div className="relative w-[90%] max-w-sm bg-white rounded-[2rem] border-[6px] border-emerald-500 shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
        ...
        {/* HEADER: Centered Title, No Close Button */}
        <div className="bg-emerald-500 p-4 flex items-center justify-center shrink-0">
          <h2 className="text-lg font-extrabold text-white font-display tracking-wider uppercase drop-shadow-md flex items-center gap-2">
              ...
              <div key={index} className="relative pl-6">
                ...
                <div className="flex flex-col">
                   <div className="flex items-center gap-2 mb-0.5">
                     <span className={`font-bold text-base ${log.current ? 'text-emerald-600' : 'text-gray-700'}`}>v{log.ver}</span>
                     {log.current && <span className="bg-emerald-100 text-emerald-600 text-[9px] font-bold px-1.5 py-0.5 rounded uppercase">{t.changelog.latest}</span>}
                   </div>
        ...
        <div className="p-4 bg-gray-50 border-t-2 border-gray-100 shrink-0 font-sans">
           <button 
             onClick={handleClose}
             className="w-full bg-emerald-500 text-white font-bold py-3 rounded-xl border-b-4 border-emerald-700 active:border-b-0 active:translate-y-1 transition-all shadow-md flex items-center justify-center gap-2 btn-3d"
           >
